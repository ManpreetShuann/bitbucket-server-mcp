...
# -- Data Center POST response shape (entities-based) -----------------------
...
class TestSearchCode:
    async def test_post_returns_normalised_results(self, setup):
        ...
        # Verify POST body has entities structure
        body = json.loads(post_route.calls[0].request.content)
        assert "entities" in body
        assert "code" in body["entities"]

    async def test_with_project_filter_in_query(self, setup):
        """Project/repo filters are embedded in the query string."""
        _, tools = setup
        with respx.mock(base_url=BASE_URL) as router:
            post_route = router.post("/rest/search/latest/search").mock(
                return_value=Response(200, json=DC_CODE_RESPONSE)
    ...
    async def test_get_fallback_with_project_filter(self, setup):
        """GET fallback still passes project.key and repository.slug as query params."""
        _, tools = setup
        with respx.mock(base_url=BASE_URL) as router:
            router.post("/rest/search/latest/search").mock(
                return_value=Response(405)
